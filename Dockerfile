...
USER root
RUN apt update \
 && apt install -y vim curl wget unzip procps simpleproxy jq less iputils-ping iproute2
RUN cd /tmp \
...
ADD lib.js services.js control.js control2.js control_in.js control_node.js control_out.js /
ENTRYPOINT [ "/bin/bash" ]
CMD [ "-c", "/control.js" ]
